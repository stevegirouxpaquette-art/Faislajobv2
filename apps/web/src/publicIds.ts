...
export function dailyPin(user:{id:string;role:'client'|'provider';client_id:string|null;provider_id:string|null},date=new Date()){
  const profile=user.role==='client'?user.client_id:user.provider_id;
  const h=fnv1a(`FLJ-verify-v1|${user.role}|${user.id}|${profile}|${localDayKey(date)}|Q7M4K9`);
