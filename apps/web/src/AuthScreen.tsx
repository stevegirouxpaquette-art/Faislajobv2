...
export default function AuthScreen({onAuthenticated}:Props){
  ...
  return <main className="premium-home-shell"><div className="premium-home-shot"><img src="/faislajob-home-final.webp" alt="Accueil FaisLaJob"/><button className="shot-hotspot shot-request" aria-label="Nouvelle demande" onClick={()=>window.location.href='/request'}></button><button className="shot-hotspot shot-login" aria-label="Se connecter" onClick={()=>openForm('login')}></button><button className="shot-hotspot shot-register" aria-label="Créer un compte" onClick={()=>openForm('register')}></button></div></main>;
}
