...
export default function App() {
  ...
  const openAuth = (mode: 'login' | 'register', role?: 'client' | 'provider') => {
    setAuthMode(mode); setAuthOpen(true); setError('');
    if (role) setAuthForm((current) => ({ ...current, role }));
  };
  ...
  const submitAuth = async () => {
    ...
    try {
      ...
      if (nextUser.role === 'provider') setPortal('provider');
        ...
        setPortal('client');
        setClient({ name: nextUser.name, email: nextUser.email, phone: nextUser.phone || '' });
      }
  ...
  const logout = async () => {
    ...
    setUser(null); setProvider(null); setOffers([]); setActiveMission(null); setPortal('client');
    localStorage.removeItem('faislajob_active_mission');
    resetFlow();
  };
  ...
  const toggleOnline = async () => {
    ...
    try {
      const response = await fetch(`${apiBase}/api/providers/${providerId}/availability`, {
  ...
  const acceptOffer = async (offerId: string) => {
    ...
    try {
      ...
      setActiveMission(mission); localStorage.setItem(`faislajob_active_mission_${providerId}`, mission.id);
      await loadPartner();
    } catch (e) { setError(e instanceof Error ? e.message : 'Erreur.'); setPartnerLoading(false); }
  };

  const declineOffer = async (offerId: string) => {
    await fetch(`${apiBase}/api/offers/${offerId}/decline`, { method: 'POST', credentials: 'same-origin' });
    await loadPartner();
  };

  const missionAction = async (action: 'en-route' | 'arrive' | 'start' | 'complete') => {
    if (!activeMission || !providerId) return;
    setPartnerLoading(true); setError('');
    try {
  ...
  const submitMission = async () => {
    ...
    try {
      ...
      if (user?.role === 'client' && user.client_id) clientId = Number(user.client_id);
        ...
        clientId = Number((await clientResponse.json()).client.id);
      }
      ...
      const missionResponse = await fetch(`${apiBase}/api/missions`, {
        method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clientId, categoryId: category.id, description }),
      });
  ...
  const authCard = (
    <section className="flow-card auth-card">
      <div className="flow-topline"><div className="eyebrow">Compte FaisLaJob</div><button className="ghost-button" onClick={() => setAuthOpen(false)}>Fermer</button></div>
      <h1 className="flow-title">{authMode === 'login' ? 'Connexion' : 'Créer un compte'}</h1>
      {authMode === 'register' && (
        <>
          <div className="role-choice">
            <button className={authForm.role === 'client' ? 'switch-active' : ''} onClick={() => setAuthForm({ ...authForm, role: 'client' })}>Je suis client</button>
            <button className={authForm.role === 'provider' ? 'switch-active' : ''} onClick={() => setAuthForm({ ...authForm, role: 'provider' })}>Je suis partenaire</button>
          </div>
          <input className="text-input" placeholder="Nom complet" value={authForm.name} onChange={(e) => setAuthForm({ ...authForm, name: e.target.value })} />
          <input className="text-input" placeholder="Téléphone" value={authForm.phone} onChange={(e) => setAuthForm({ ...authForm, phone: e.target.value })} />
        </>
      )}
      <input className="text-input" type="email" placeholder="Courriel" value={authForm.email} onChange={(e) => setAuthForm({ ...authForm, email: e.target.value })} />
      <input className="text-input" type="password" placeholder="Mot de passe (8 caractères minimum)" value={authForm.password} onChange={(e) => setAuthForm({ ...authForm, password: e.target.value })} />
      {error && <p className="error-box">⚠️ {error}</p>}
      <button className="primary-button full-button" disabled={authLoading} onClick={submitAuth}>{authLoading ? 'Un instant…' : authMode === 'login' ? 'Me connecter' : 'Créer mon compte'}</button>
      <button className="link-button" onClick={() => { setAuthMode(authMode === 'login' ? 'register' : 'login'); setError(''); }}>{authMode === 'login' ? 'Pas encore de compte? Créer un compte' : 'J’ai déjà un compte'}</button>
    </section>
  );
    ...
    <main className="page-shell">
      <header className="topbar">
        ...
        <div className="header-actions">
          ...
          {user ? <><span className="user-chip">{user.name}</span><button className="ghost-button" onClick={logout}>Déconnexion</button></> : <button className="ghost-button" onClick={() => openAuth('login')}>Connexion</button>}
        </div>
      </header>
      ...
      {authOpen ? authCard : portal === 'provider' ? (
        user?.role !== 'provider' ? (
          <section className="flow-card">
            <div className="eyebrow">Portail partenaire</div>
            <h1 className="flow-title">Connecte-toi comme partenaire</h1>
            <p className="flow-copy">Ton compte partenaire remplace maintenant le prestataire test. Tes offres et tes missions sont liées à ton vrai profil.</p>
            <button className="primary-button full-button" onClick={() => openAuth('login', 'provider')}>Connexion partenaire</button>
            <button className="link-button" onClick={() => openAuth('register', 'provider')}>Créer mon compte partenaire</button>
          </section>
        ) : (
          <section className="flow-card">
            <div className="flow-topline"><div className="eyebrow">Portail partenaire</div><button className="ghost-button" onClick={loadPartner}>Actualiser</button></div>
            <h1 className="flow-title">{provider?.name || user.name}</h1>
            {error && <p className="error-box">⚠️ {error}</p>}
        ...
        <>
          <section className="flow-card">
            ...
            {user?.role === 'client' && step === 0 && <p className="account-note">Connecté comme <strong>{user.name}</strong> — ta demande sera liée automatiquement à ton compte.</p>}
          ...
          <section className="partner-card compact-partner-card"><div><div className="eyebrow">Tu veux faire des jobs?</div><h2>Travaille quand tu veux.</h2></div><button className="secondary-button" onClick={() => { setPortal('provider'); if (!user) openAuth('register', 'provider'); }}>Devenir partenaire</button></section>
        </>
      )}
    </main>
  );
