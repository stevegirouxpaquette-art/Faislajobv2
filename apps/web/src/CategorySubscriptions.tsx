...
type Plan={id:string;name:string;description?:string|null;trigger_type:'snow_accumulation'|'weekly'|'interval'|'manual';trigger_config:any;pricing_type:'per_visit'|'monthly'|'seasonal';price_cents:number;same_team:boolean;max_interventions?:number|null};
...
export default function CategorySubscriptions({categoryId}:{categoryId:string}){
 ...
 const price=(p:Plan)=>`${money(p.price_cents)} ${p.pricing_type==='per_visit'?'par intervention':p.pricing_type==='monthly'?'par mois':'par saison'}`;
