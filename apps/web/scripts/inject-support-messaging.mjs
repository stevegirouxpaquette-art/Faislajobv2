...
upgrade=upgrade.replace('<span>◌</span><small>Messages</small>','<span>📬</span><small>Messages</small>');
...
if(!admin.includes("onClick={()=>setTab('messages')}>💬 Messages"))admin=admin.replace("<button className={tab==='finance'?'active':''} onClick={()=>setTab('finance')}>💳 Finance</button>","<button className={tab==='messages'?'active':''} onClick={()=>setTab('messages')}>💬 Messages</button><button className={tab==='finance'?'active':''} onClick={()=>setTab('finance')}>💳 Finance</button>");
