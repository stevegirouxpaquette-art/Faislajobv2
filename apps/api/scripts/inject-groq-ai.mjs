...
const previousPrompt = `Tu es l'assistant de FaisLaJob, une plateforme québécoise de petits travaux et services à domicile. À partir de la demande du client, prépare une checklist claire pour le prestataire. N'invente pas de travaux importants qui n'ont pas été demandés. Les tâches doivent être courtes, concrètes et cochables. Si une information essentielle manque pour bien définir la mission, ajoute-la dans questions. Réponds en français québécois clair et professionnel.`;
const strictPrompt = `Tu es l'assistant de FaisLaJob, une plateforme québécoise de petits travaux et services à domicile.

RÈGLE ABSOLUE : la checklist doit contenir UNIQUEMENT les tâches explicitement demandées par le client. N'ajoute jamais une tâche, une sous-tâche, une surface, un appareil, un produit, une méthode ou un service simplement parce que c'est habituellement inclus dans ce type de travail.

Si le client dit « nettoyer la salle de bain », crée seulement une tâche « Nettoyer la salle de bain ». Ne transforme pas cette demande en « nettoyer la baignoire », « nettoyer la douche », « nettoyer la toilette », etc., à moins que le client les ait nommés lui-même. Même règle pour la cuisine, le ménage, le déneigement, le déménagement et toutes les autres catégories.

Le champ details peut reformuler ou préciser ce que le client a explicitement dit, mais il ne doit jamais élargir la portée de la tâche. Si une précision, une option, un extra ou une information essentielle manque, mets-la uniquement dans questions sous forme de question au client. En cas de doute, pose une question au lieu de faire une supposition.

Le summary doit seulement résumer la demande exprimée. Les tâches doivent être courtes, concrètes et cochables. Réponds en français québécois clair et professionnel.`;

// Upgrade an already-injected server from the original prompt to the strict prompt.
if (source.includes(previousPrompt)) source = source.replace(previousPrompt, strictPrompt);

const routeMarker = '// ADMIN / DISPATCH';
const routeBlock = `// GROQ AI - turn a client's description into a structured task checklist\napp.post('/api/ai/task-list',async(request,reply)=>{\n  const u=await requireRole(request,reply,'client');if(!u)return;\n  if(!GROQ_API_KEY)return reply.code(503).send({error:'AI is not configured on the server'});\n  const b=request.body as any,description=String(b?.description||'').trim(),category=String(b?.category||'General').trim();\n  if(description.length<3)return reply.code(400).send({error:'description is required'});\n  if(description.length>4000)return reply.code(400).send({error:'description is too long'});\n  try{\n    const response=await fetch('https://api.groq.com/openai/v1/chat/completions',{\n      method:'POST',\n      headers:{'Authorization':\`Bearer \${GROQ_API_KEY}\`,'Content-Type':'application/json'},\n      signal:AbortSignal.timeout(20000),\n      body:JSON.stringify({\n        model:GROQ_MODEL,\n        reasoning_effort:'low',\n        messages:[\n          {role:'system',content:${JSON.stringify(strictPrompt)}},\n          {role:'user',content:\`Catégorie: \${category}\\nDemande du client: \${description}\`}\n        ],\n        response_format:{\n          type:'json_schema',\n          json_schema:{\n            name:'faislajob_task_list',\n            strict:true,\n            schema:{\n              type:'object',\n              properties:{\n                summary:{type:'string'},\n                tasks:{type:'array',items:{type:'object',properties:{title:{type:'string'},details:{type:'string'},required:{type:'boolean'}},required:['title','details','required'],additionalProperties:false}},\n                questions:{type:'array',items:{type:'string'}}\n              },\n              required:['summary','tasks','questions'],\n              additionalProperties:false\n            }\n          }\n        }\n      })\n    });\n    const data=await response.json() as any;\n    if(!response.ok){app.log.error({status:response.status,groq:data},'Groq request failed');return reply.code(502).send({error:'AI provider error',status:response.status});}\n    const content=data?.choices?.[0]?.message?.content;\n    if(!content)return reply.code(502).send({error:'AI returned an empty response'});\n    const result=JSON.parse(content);\n    return{ok:true,model:data?.model||GROQ_MODEL,result,usage:data?.usage??null};\n  }catch(e){app.log.error(e);return reply.code(502).send({error:'AI request failed'});}\n});\n\n${routeMarker}`;
