...
const marker = '// CATEGORY APPROVAL V3';
if (source.includes(marker)) {
  console.log('✓ category approval already wired');
  process.exit(0);
}

const childcareLine = "  { id: 'garde-enfant-devoirs', name: 'Garde d’enfant & aide aux devoirs', hourlyRateCents: 3000 }, // 0,50 $/min\n";
const unclassifiedLine = "  { id: 'a-classer', name: 'Autre demande — approbation admin', hourlyRateCents: 4000 }, // 0,67 $/min\n";
if (!source.includes("{ id: 'garde-enfant-devoirs', name:")) throw new Error('category approval: childcare category not found');
if (!source.includes("{ id: 'a-classer', name:")) {
  const lastCategory = "  { id: 'animaux', name: 'Animaux', hourlyRateCents: 2700 }, // 0,45 $/min\n";
  if (!source.includes(lastCategory)) throw new Error('category approval: default category anchor not found');
  source = source.replace(lastCategory, `${lastCategory}${unclassifiedLine}`);
}

const categoriesRoute = "app.get('/api/categories',async()=>({categories:(await pool.query('SELECT id,name,hourly_rate_cents FROM categories ORDER BY name')).rows}));";
if (source.includes(categoriesRoute)) {
  source = source.replace(categoriesRoute, "app.get('/api/categories',async()=>({categories:(await pool.query(\"SELECT id,name,hourly_rate_cents FROM categories WHERE id <> 'a-classer' ORDER BY name\")).rows}));");
} else if (!source.includes("WHERE id <> 'a-classer'")) {
  throw new Error('category approval: categories route not found');
}
...
if (!source.includes("app.post('/api/missions'")) throw new Error('category approval: mission route not found');
if (!source.includes("pending_admin_category")) {
  if (!oldMissionRoute.test(source)) throw new Error('category approval: mission route pattern not found');
